...
6. `core/v8-rct-model.js`：V8.1 反转术式掌握度、治疗、伤势可修复性与 AI 治疗阈值纯模型
7. `core/v8-app.js`：V8 与人生转盘的集成层
8. `core/v8-rct.js`：把 RCT 模型接入战斗牌池、CPU、行动结算、战后伤势和命运面板

V7 不被推翻。V8 在其上增加“命运、选择与后果”，V8.1 开始让治疗和失败后果真正参与人生。
...
首个接入的关键历史节点是涩谷事变，用来验证“玩家选择 → 后续权重改变 → 世界状态继续传播”整条链。

## V8.1：反转术式与伤势

反转术式不再只是 `hasRCT = true` 或一张固定小回血牌。
...
掌握度由明确能力描述以及咒力操纵、效率、天赋、术式能力共同决定。不同层级逐步解锁：
...
`反转术式外放 / 正能量外放` 是独立能力，不因会自愈而自动获得。原本战斗引擎已有的正能量对咒灵特殊结算继续保留。

战斗中的「反转术式疗伤」现在按最大体势和掌握度计算，不再只恢复固定 16 / 20 / 24 点。当前目标量级约为最大体势的 14% / 20% / 28% / 36%，同时仍消耗 CE 和 1 AP。

CPU 会根据自身 HP、剩余 CE、RCT 熟练度、敌方斩杀线和治疗冷却判断是否疗伤，不再只因为“有治疗牌”就随缘使用。
...
- 高级 RCT 才允许断肢再生
- 普通 RCT 不能自动洗掉灵魂损伤
- 术式熔断修复必须是顶尖 RCT 且明确解锁该能力
- RCT 不自动复活，也不会把所有永久后果无条件抹除

如果高阶 RCT 在伤势结算时完整修复了一个明确的永久属性下降，系统会阻止该次永久降阶继续污染后续战斗状态；部分修复则只登记为稳定伤势，不直接洗掉永久代价。
...
| `core/v8-rct-model.js` | V8.1 RCT 纯规则模型 |
| `core/v8-app.js` | V8 人生转盘集成与 UI |
| `core/v8-rct.js` | V8.1 RCT 运行时接入层 |
...
node --check core/v8-rct-model.js
node --check core/v8-app.js
node --check core/v8-rct.js
node tests/v7-model.test.js
node tests/v8-state.test.js
node tests/v8-rct.test.js
...
- V8 RCT Model 只定义治疗与伤势规则，不复制 V7 伤害公式。
- V8 App 负责“状态如何接入人生转盘、关键抉择与 UI”。
