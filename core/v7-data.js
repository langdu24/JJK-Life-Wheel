(() => {
  ...
  function clone(v) {
    try { return structuredClone(v); } catch {
      try { return JSON.parse(JSON.stringify(v)); } catch { return v && typeof v === "object" ? { ...v } : v; }
    }
  }
  function profileOf(v) { return v?.characterCardProfile || v?.profile || v || {}; }
  function profileText(v) {
    const p = profileOf(v);
    return [v?.id, v?.name, p.id, p.characterId, p.displayName, p.name, p.visibleGrade, p.officialGrade, p.powerTier, p.grade, p.gradeLabel, p.notes, ...arr(p.traits), ...arr(p.innateTraits), ...arr(p.cardTags), ...arr(p.specialHandTags)].filter(Boolean).join(" ");
  }
  function isLifeWheelProfile(p) {
    p = p || {};
    const t = [p.displayName, p.name, p.notes, p.source, p.sourceType].filter(Boolean).join(" ");
    return /^战力导入：/.test(String(p.displayName || p.name || "")) || /life[-_ ]?wheel|人生转盘|转盘导入/i.test(t) || Boolean(p.encodedCombatProfile);
  }
  function isZeroCe(p) {
    const f = p?.flags;
    const ft = Array.isArray(f) ? f.join(" ") : f && typeof f === "object" ? Object.keys(f).filter(k => f[k]).join(" ") : String(f || "");
    return /zero.?ce|零咒力|天与咒缚（肉体强化型）|天与暴君|domainSureHitInvalid/i.test(`${profileText(p)} ${ft}`);
  }
  function detectNoInnateTechnique(p) {
    ...
    if (isLifeWheelProfile(p) && !p.techniquePower && !p.technique && !(p.techniques && p.techniques.length) && /准一级|一级|二级|三级|特级/.test(text) && !/生得术式|固有术式|innate technique/i.test(text)) return true;
    return false;
  }
  function detectGrade(p) {
    p = p || {};
    const direct = p.gradeRank ?? p.grade ?? p.gradeLabel ?? p.visibleGrade ?? p.officialGrade ?? p.powerTier;
    const fromDirect = M.parseGrade(direct);
    if (fromDirect > 0) return fromDirect;
    const fromText = M.parseGrade(profileText(p));
    if (fromText > 0) return fromText;
    if (typeof state !== "undefined") {
      const flagGrade = M.parseGrade(state?.flags?.sorcererGradeLabel || state?.flags?.sorcererGrade || "");
  ...
  function readTechniquePower(p) {
    ...
    for (const v of [p.techniquePower, p.technique?.power, p.techniques?.[0]?.power, p.raw?.techniquePower]) {
  ...
  function transcendenceFor(p) {
    ...
    const ids = ["cursedEnergy", "control", "efficiency", "martial", "body", "talent", "techniquePower"];
    return Object.fromEntries(ids.map(id => [id, Math.max(0, Math.trunc(num(b[id] ?? a[id], 0)))]));
  }
  function model(p) {
    p = p || {};
  ...
  function actionText(a) {
    return [a?.id, a?.actionId, a?.cardId, a?.name, a?.label, a?.type, a?.cardType, a?.damageType, a?.scalingProfile, a?.risk, a?.effectSummary, ...arr(a?.tags), ...arr(a?.specialHandTags)].filter(Boolean).join(" ").toLowerCase();
  }
  function actionTier(a) { return M.tierFromText(actionText(a), a?.risk); }
  function actionArchetype(a) {
    const t = actionText(a);
    if (/heal|healing|rct|reverse|治疗|疗伤|回复|support|resource|防御|guard|block/.test(t)) return "utility";
    const p = /melee|physical|martial|体术|近身|拳|踢|咒具|cursed_tool|weapon|刀|剑|chain/.test(t);
    const q = /technique|innate|术式|咒法|curse|beam|projectile|domain|领域|必中|blue|red|purple|苍|赫|茈|御厨子|shrine|maximum/.test(t);
  ...
  function directTechniqueMultiplier(actor, card) {
    const w = globalThis.JJKBattleDataDirect?.getBattleCardCoreWeights?.(card) || { martial: 0, technique: 1 };
  ...
  function normalizeScaling(card) {
    ...
    c.scaling = { ...c.scaling };
    const s = c.scaling;
    s.bodyResistanceBase = 0.1; s.bodyResistancePerRank = 0.025; s.bodyResistanceMin = 0.025; s.bodyResistanceMax = 0.42;
    s.martialEvasionPerRank = 0.025; s.efficiencyCostPerRank = 0.07; s.efficiencyCostMin = 0.48; s.efficiencyCostMax = 1.55;
    s.sourceDamageMin = Math.min(num(s.sourceDamageMin, 0.25), 0.25);
    s.sourceDamageMax = Math.max(num(s.sourceDamageMax, 2.5), 3.2);
    return c;
  }
  function isStoryBattle(b) { return String(b?.activityContext?.type || "") === STORY_TYPE; }
  function isPlayerActor(a) { return String(a?.side || "") === "left" || isLifeWheelProfile(profileOf(a)); }
  function scaleDamageFields(a, m) {
    m = num(m, 1);
    for (const [o, k] of [[a, "damage"], [a, "baseDamage"], [a?.effect, "damage"]]) {
      if (o && Number.isFinite(Number(o[k])) && Number(o[k]) > 0) o[k] = Number((Number(o[k]) * m).toFixed(4));
    }
  }
  function neutralizeEmbeddedTojiHotfix(a, actor) {
    const id = String(a?.id || a?.actionId || a?.cardId || "");
    if (!LEGACY_TOJI_HEAVY.has(id) || !/伏黑甚尔（怀玉|toji_fushiguro_hidden_inventory/i.test(profileText(actor))) return a;
    const c = clone(a);
  ...
  function applyMatchupToLoss(loss, attacker, defender) {
    ...
    return { adjusted: Math.max(0, loss * mult), mult, attackerModel: am, defenderModel: dm };
  }
  function installResourceModel() {
    ...
    const w = function (p, c, side) {
      ...
      if (!r) return r;
      const m = model(p);
      r.maxHp = m.maxHp; r.hp = m.maxHp; r.maxCe = m.maxCe; r.ce = m.maxCe;
      r.ceCostMultiplier = m.ceCostMultiplier;
      if (m.explicitNoCT && m.gradeRank >= 4) r.ceCostMultiplier = Number((r.ceCostMultiplier * 0.9).toFixed(4));
      if (m.utilityRole && m.gradeRank > 0 && m.gradeRank <= 2) r.ceCostMultiplier = Number((r.ceCostMultiplier * 1.08).toFixed(4));
      r.bodyResistance = m.bodyResistance;
      r.regenRatio = m.zeroCe ? 0 : 0.02 + 0.11 * (1 - Math.exp(-(m.control + m.efficiency) / 16));
      r.ceRegen = m.zeroCe ? 0 : Number((r.maxCe * r.regenRatio).toFixed(1));
      r.v7Model = { ...m };
      r.raw = { ...(r.raw || {}), cursedEnergyScore: m.cursedEnergy, controlScore: m.control, efficiencyScore: m.efficiency, martialScore: m.martial, bodyScore: m.body, talentScore: m.talent, gradeRank: m.gradeRank, explicitNoCT: m.explicitNoCT };
      return r;
    };
  ...
  function installDirectCardModel() {
    ...
    const w = function (card, actor, opponent, options) {
      ...
      const match = M.matchupMultiplier(am.gradeRank, dm.gradeRank, { attackerNoCT: am.explicitNoCT, defenderUtility: dm.utilityRole, defenderNoCT: dm.explicitNoCT });
    ...
    Object.defineProperty(w, "__jjkV7Data", { value: true });
    try { globalThis.JJKBattleDataDirect = Object.freeze({ ...api, calculateBattleCardSettlement: w }); return true; }
    catch { try { api.calculateBattleCardSettlement = w; return true; } catch { return false; } }
  }
    ...
    const w = function (action, actor, opponent, battle) {
      ...
      if (isStoryBattle(battle) && isPlayerActor(actor)) {
        ...
        if (!a.scaling && a.type !== "card_template_runtime") {
          ...
          const ratio = M.projectionRatio(tier, { noCT: actorModel.explicitNoCT, gradeRank: actorModel.gradeRank });
          a.projectionRuntime = { ...(a.projectionRuntime || {}), active: true, damageSettlementRatio: ratio };
          a.projectionDamageSettlementRatio = ratio;
          const mult = genericTechniqueMultiplier(p, a);
          if (Math.abs(mult - 1) >= 0.002) { scaleDamageFields(a, mult); a.v7TechniquePowerMultiplier = Number(mult.toFixed(4)); }
          const defModel = model(profileOf(opponent));
          const match = M.matchupMultiplier(actorModel.gradeRank, defModel.gradeRank, { attackerNoCT: actorModel.explicitNoCT, defenderUtility: defModel.utilityRole });
          if (match > 1.02) { scaleDamageFields(a, match); a.v7MatchupMultiplier = Number(match.toFixed(4)); }
        }
      }
      ...
      const soft = Math.min(before, M.softBurstDamage(loss, maxHp, actionTier(a), matchInfo.attackerModel.combatRating, matchInfo.defenderModel.combatRating));
      if (soft + 0.5 < before - after || Math.abs(matchInfo.mult - 1) >= 0.02) {
        ...
        try {
          (battle?.resourceLog || battle?.log)?.unshift?.({
            ...
            type: "system",
            delta: { v7Matchup: true, mult: matchInfo.mult, rawLoss: before - after, softened: soft }
          });
        } catch {}
      }
      return r;
  ...
  function installAll() {
    return Object.freeze({ resource: installResourceModel(), directCards: installDirectCardModel(), actions: installActionModel() });
  }
  function ready(r) { return Boolean(r.resource && r.directCards && r.actions); }
  let attempts = 0;
  function boot() {
    ...
    if (ready(report)) {
      ...
      try { document.dispatchEvent(new CustomEvent("jjk:v7-data-ready", { detail: { version: VERSION, report } })); } catch {}
      return;
    }
    attempts += 1;
  ...
  globalThis.JJKV7Data = { version: VERSION, model, detectGrade, detectNoInnateTechnique, detectUtilityRole, installAll, installReport: null, actionTier, actionArchetype };
  boot();
})();
